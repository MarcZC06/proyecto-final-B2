...
Los desafíos clave incluyen las posibles variedades de los datos (formatos, nomenclaturas, tipos, etc...), que pueda haber duplicados o datos inconsistentes, que puedan faltar valores, y asegurar la privacidad/confidencialidad al compartir y procesar datos entre entidades.
...
Permite a los usuarios acceder, comparar y compartir sus datos entre instituciones, lo que fomenta la competencia, facilita mejores ofertas y ayuda a los usuarios a tomar decisiones más informadas sobre productos y servicios financieros.
...
Todos estos modelos abren los datos para facilitar su intercambio entre entidades, promoviendo transparencia, competencia y mayor acceso a productos personalizados para los usuarios. La diferencia es el alcance: Open Banking es solo bancario, Open Insurance de seguros, y Open Finance engloba ambos y más.
...
Tipos de columnas, presencia de valores faltantes, duplicados, formatos de datos, distribuciones de variables, outliers y correlaciones. Problemas como estos, pueden provocar que los modelos de machine learning sean poco precisos o tengan desbalances.
## Exploración Inicial
...
En los 3 archivos .csv se puede observar que el atributo común es "ID".
"""
#Write your code here
...
#Write your code here
print("Registros RetailBankEFG:", len(df_retailbank))
print("Registros InvestmentBankCDE:", len(df_investment))
print("Registros InsuranceCompanyABC:", len(df_insurance))

"Podemos ver si hay clientes en todos los bancos o compañías de seguros."
"Si el número de registros es muy diferente entre los conjuntos, puede haber clientes exclusivos de un banco o compañía."
"""## Pregunta
¿Has notado algún patrón entre los datos, ya sea entre filas o columnas?
Valores faltantes en ciertas columnas, presencia de algunos valores extremos (outliers), o diferencias en el formato de los datos (decimales con coma/punto).
...
Si, si los valores son mayores a 0, en caso de no serlo, no hay dplicados en el archivo
## Inconsistencias
...
#Write your code here for df_retailbank
print("Descripción RetailBankEFG:\n", df_retailbank.describe(include='all'))

#Write your code here for df_investment
print("Descripción InvestmentBankCDE:\n", df_investment.describe(include='all'))

#Write your code here for df_insurance
print("Descripción InsuranceCompanyABC:\n", df_insurance.describe(include='all'))
...
Algunas pueden tener valores que no están representados igual, por ejemplo, a vaeces se escribe T/F y otras True/false
...
for col in df_retailbank.select_dtypes(include=np.number).columns:
    if col != "ID":
        plt.figure()
        sns.boxplot(x=df_retailbank[col])
        plt.title(f'Boxplot {col} - RetailBankEFG')
        plt.show()
#Write your code here, add your custom plots for df_investment
for col in df_investment.select_dtypes(include=np.number).columns:
    if col != "ID":
        plt.figure()
        sns.boxplot(x=df_investment[col])
        plt.title(f'Boxplot {col} - InvestmentBankCDE')
        plt.show()
...
for col in df_insurance.select_dtypes(include=np.number).columns:
    if col != "ID":
        ...
        plt.title(f'Boxplot {col} - InsuranceCompanyABC')
        plt.show()
"""## Preguntas
1. *¿Cuál de las dos opciones sugieres utilizar para evaluar datos no numéricos: imprimir los valores o crear visualizaciones?* Visualizacones para ver errores de valores no numéricos
2. *¿Qué otros tipos de visualizaciones se te ocurren que podrías sugerir? Justifica tu respuesta.* Histogramas, gráficos de barras, diagramas de dispersión para relaciones entre variables, y mapas de calor de correlaciones. Ayudan a descubrir patrones, outliers y relaciones entre variables.
3. *¿Existe un desbalance en los datos, es decir, existen más tipos que corresponden a una clase? ¿Cuál es la clase y cómo crees que esto puede afectar al construir modelos de machine learning?* La variable objetivo está 
desbalanceada. Si una clase es mayoritaria, los modelos tenderán a predecir esa clase más a menudo, afectando la precisión para las clases minoritarias. Por eso se debería de balancear.
...
"""## Preguntas
* *¿Cuál es la distribución de datos sugerida?* Las boxplots y violinplots muestran la mediana, los cuartiles y los valores extremos por región, permitiendo observar si hay regiones con edades o rentas significativamente más altas o bajas.
* *¿Existen datos atípicos en el conjunto de datos?* *¿Cómo podrías corregir estos datos? Justifica tu respuesta*. Sí, los boxplots muestran outliers. Se pueden corregir eliminando, pero si representan casos reales, podría no ser conveniente eliminarlos. Para algunos modelos, a veces se eliminan los muy extremos.
...
Los datos duplicados pueden provocar que nuestro código sufra alteraciones en los resultados, haciendo que sean menos precisos
...
#Write your code here
df_insurance = OutlierRemover(columns=['Idade', 'Renda']).fit_transform(df_insurance)
...
#Write your code here
print("Filas después de eliminar outliers:", df_insurance.shape[0])
...
Si eliminamos muchos datos, es mñas dificil que el modelo aprenda patrones representativos
...
MinMaxScaler (valores a un rango) y RobustScaler
...
# Write you code here
data_frame_merged = df_retailbank.merge(df_investment, on="ID").merge(df_insurance, on="ID")

"""*Imprime la cantidad total de registros después de realizar el merge entre los conjuntos de datos.*"""

# Write you code here
print("Total de registros tras merge:", len(data_frame_merged))

"""*Observamos una visión estadística rápida de los datos mediante la función `describe`.*"""

# Write you code here
print(data_frame_merged.describe(include='all'))
...
# Write you code here
print(data_frame_tipo_financiamiento.describe(include='all'))
...
plt.title("Proporción de tipos de financiamiento")
plt.show()
...
Se puede suponer, ya que Random Forest y Gradient SUELEN tener un mejor rendimiento, pero no se puede saber al 100% creo yo
...
Quizá con exactitud no, por los desbalances. Harbría que ajustar aloritmos y variables
...
top6 = get_most_important_features(corr_matrix, "tipo_financiamiento", n=6)
print("Top 6 variables más relevantes:", top6)

"""# Análisis de Componentes Principales(PCA)

## Pregunta
*¿Qué es el análisis de componentes principales y cuál es su utilidad al implementar modelos de machine learning?*
Es una técnica de reducción que transforma variables originales en nuevas variables. Útil para que el modelo sea menos complejo.
...
Hay coincidencias: las variables que más varianza muestran en PCA suelen ser también las que tienen mayor correlación con la variable objetivo.
...
Muestra cuántos componentes son óptimos para retener la mayor varianza. Normalmente se elige el número donde la pendiente se aplana, a partir de ahí añadir más componentes da poca ganancia. En este caso suelen ser 2 o 3
...
plt.ylabel('')
plt.title("Proporción original tipos de financiamiento")
plt.show()
print(y.value_counts())

"""*Separa los datos en conjuntos de entrenamiento y test utilizando la función `startified_train_test_split()`. Luego, implementa un modelo que haga uso del siguiente clasificador. Puedes probar modificando los hiperparámetros y evaluar los resultados. 
También puedes optar por modificar los parámetros de las clases `RandomUnderSampler` y `SMOTE` del paso anterior.*
...
X_train_bal, X_test_bal, y_train_bal, y_test_bal = startified_train_test_split(
    pd.DataFrame(X_reshaped, columns=X.columns),
    pd.Series(y_reshaped),
    test_size=0.2,
    random_state=42
)
...
pipeline_balanced = Pipeline(steps_gradient_boost)

# Entrenar el modelo
pipeline_balanced.fit(X_train_bal, y_train_bal)

# Hacer predicciones
y_pred_bal = pipeline_balanced.predict(X_test_bal)

# Evaluar el modelo
from sklearn.metrics import classification_report, confusion_matrix

print("Reporte clasificación Gradient Boosting balanceado:\n",
      classification_report(y_test_bal, y_pred_bal))
plot_confusion_matrix(confusion_matrix(y_test_bal, y_pred_bal), tipo_financiamiento_mapping)
...
Gradient Boosting, da mejor rendimiento total
...
Gradient Boosting, a parte de la precisión global, tiene una buena sensibilidad para las clases minoritarias
...
print("Distribución tras ADASYN:", pd.Series(y_ada).value_counts())

* *Investiga qué son los modelos de ensamble e implementa un corto ejemplo.*
Combinan varios modelos base y así se obtiene un resultado más robusto y preciso.
...
print("Accuracy VotingClassifier:", accuracy_score(y_test, y_pred_vc))
"""